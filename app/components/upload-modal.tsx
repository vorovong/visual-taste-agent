...
export function UploadModal({ open, onClose }: UploadModalProps) {
    ...
    <div className="flex items-center gap-3 rounded-xl bg-white/[0.04] border border-white/[0.06] px-4 py-3">
      ...
      {state === "idle" && (
        <button
          ...
          className="shrink-0 rounded-lg p-1.5 text-neutral-500 hover:text-neutral-300 hover:bg-white/[0.06] transition-colors"
        >
  ...
  return (
    ...
    >
      ...
      {/* Modal */}
      <div
        className="relative w-full max-w-md rounded-2xl border border-white/[0.08] bg-neutral-900 shadow-2xl shadow-black/50 overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between border-b border-white/[0.06] px-5 py-4">
          <h2 className="text-sm font-semibold text-neutral-200">파일 업로드</h2>
          <button
            onClick={handleClose}
            className="rounded-lg p-1 text-neutral-500 hover:text-neutral-300 hover:bg-white/[0.06] transition-colors"
          >
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          ...
          {state === "success" ? (
            ...
            <div className="flex flex-col items-center py-6 space-y-4">
              ...
              <div className="text-center">
                <p className="text-sm font-medium text-neutral-200">업로드 완료</p>
                <p className="mt-1 text-xs text-neutral-500">파일이 갤러리에 추가되었습니다</p>
              </div>
              <div className="flex gap-2 pt-2">
                <button
                  onClick={() => reset()}
                  className="rounded-xl px-4 py-2 text-xs font-medium text-neutral-400 hover:text-neutral-200 hover:bg-white/[0.06] transition-colors"
                >
                  추가 업로드
                </button>
                <button
              ...
              {!file && (
                ...
                >
                  ...
                  <p className="text-sm text-neutral-400">
                    드래그 앤 드롭 또는{" "}
                    <span className="text-emerald-400 font-medium">파일 선택</span>
                  </p>
              ...
              {file && (
                <div className="space-y-1.5">
                  ...
                  <select
                    ...
                    className="w-full rounded-xl border border-white/[0.08] bg-white/[0.04] px-3 py-2.5 text-sm text-neutral-200 outline-none focus:border-emerald-500/40 transition-colors appearance-none cursor-pointer"
                  >
                    {CONTENT_TYPES.map((ct) => (
                      <option key={ct.value} value={ct.value} className="bg-neutral-900">
                        {ct.label}
                      </option>
                    ))}
                  </select>
